...
void mutate(BrainLoss &brainScore, vector<NeuralNetwork> &brains) {
	...
	for (int i = 0; i < brain.getDepth(); i++) {
		if (brain.getLayer(i)->type == OPENCL_FULLY_CONNECTED) {
			OpenCLFullyConnectedLayer *layer = (OpenCLFullyConnectedLayer *) brain.getLayer(i);
			vector<vector<weight> > weights = layer->getNeuronWeights();
			for (int j = 0; j < layer->outputSize; j++) {
				Neuron::mutateWeights(weights[j], brainScore.mutationRate);
			}
			layer->setNeuronWeights(weights);
		}
	}
}

int main(int argc, char *argv[]) {
	OpenCLFullyConnectedLayer::staticInit();
	...
	OpenCLFullyConnectedLayer fullyConnectedLayer3(weights3);
	...
	Neuron::randomizeWeights(weights1);
	OpenCLFullyConnectedLayer fullyConnectedLayer1_(weights1);
	trueBrain.addLayer((Layer &) fullyConnectedLayer1_);
	Neuron::randomizeWeights(weights2);
	OpenCLFullyConnectedLayer fullyConnectedLayer2_(weights2);
	trueBrain.addLayer((Layer &) fullyConnectedLayer2_);
	Neuron::randomizeWeights(weights3);
	OpenCLFullyConnectedLayer fullyConnectedLayer3_(weights3);
...
//			cout << endl;
	...
	OpenCLFullyConnectedLayer::staticDeinit();
	return 0;
}
